...
describeWithFrontier("Frontier RPC (Fee History)", (context) => {
  ...
  function getPercentile(percentile, array) {
    ...
    if (Math.floor(index) == index) {
      ...
      return Math.ceil(
        (array[Math.floor(index)] + array[Math.ceil(index)]) / 2,
      );
    }
  }
    ...
    for (var b = 0; b < block_count; b++) {
      for (var p = 0; p < priority_fees.length; p++) {
        await sendTransaction(context, {
          ...
          maxPriorityFeePerGas: context.web3.utils.numberToHex(
            priority_fees[p],
          ),
          accessList: [],
          nonce: nonce,
  ...
  step("should return error on non-existent blocks", async function () {
    this.timeout(100000);
    let result = customRequest(context.web3, "eth_feeHistory", [
      "0x0",
      "0x1",
      [],
    ])
  ...
  step("result length should match spec", async function () {
    ...
    expect(result.gasUsedRatio).to.be.deep.eq(
      Array(blockCount).fill(0.03576792),
    );
