...
describeWithFrontier("Frontier RPC (RPC execution)", (context) => {
    ...
    const result = await customRequest(context.web3, "eth_call", [
      {
        ...
        data: contract.methods
          .force_gas(block.gasLimit * 10 - 500_000)
          .encodeABI(),
      },
    ]);
  ...
  step(
    "Deserializes correctly when data and input are equal",
    async function () {
      const result = await customRequest(context.web3, "eth_call", [
        {
          from: GENESIS_ACCOUNT,
  ...
  step(
    "Throws error when data and input are both present and not equal",
    async function () {
      const result = await customRequest(context.web3, "eth_call", [
        {
          from: GENESIS_ACCOUNT,
      ...
      expect(result.error.data).to.match(
        /^Ambiguous value for `data` and `input`/,
      );
    },
  );
});
