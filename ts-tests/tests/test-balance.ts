...
describeWithFrontier("Frontier RPC (Balance)", (context) => {
  ...
  const TRANSFER_VALUE = "0x2000"; // must be higher than ExistentialDeposit
  ...
  step("balance to be updated after transfer", async function () {
    ...
    const expectedGenesisBalance = (
      ...
      BigInt(TRANSFER_VALUE)
    ).toString();
    const expectedTestBalance = (
      Number(TRANSFER_VALUE) - EXISTENTIAL_DEPOSIT
    ).toString();
    expect(
      await context.web3.eth.getBalance(GENESIS_ACCOUNT, "pending"),
    ).to.equal(expectedGenesisBalance);
    expect(await context.web3.eth.getBalance(TEST_ACCOUNT, "pending")).to.equal(
      expectedTestBalance,
    );
