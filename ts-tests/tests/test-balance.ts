...
describeWithFrontier("Frontier RPC (Balance)", (context) => {
	...
	step("balance to be updated after transfer", async function () {
		...
		const expectedGenesisBalance = (
			BigInt(GENESIS_ACCOUNT_BALANCE) - BigInt(21000) * BigInt(GAS_PRICE) - BigInt(TRANSFER_VALUE)
		).toString();
		const expectedTestBalance = (Number(TRANSFER_VALUE) - EXISTENTIAL_DEPOSIT).toString();
