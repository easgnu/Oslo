...
import {
  ...
  customRequest,
  describeWithFrontierAllPools,
} from "./util";

describeWithFrontierAllPools(
  "Frontier RPC (Pending Transactions)",
  (context) => {
    ...
    step(
      "should return empty array when no transactions are pending",
      async function () {
    ...
    step(
      "should return pending transactions when transactions are in mempool",
      async function () {
        ...
        // Get initial nonce
        const initialNonce =
          await context.web3.eth.getTransactionCount(GENESIS_ACCOUNT);
    ...
    step(
      "should remove transactions from pending transactions when block is created",
      async function () {
        // First, create a block to clear previous pending transactions
        await createAndFinalizeBlock(context.web3);
