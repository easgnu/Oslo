...
function binarySearch(oneOffEstimation) {
  ...
  while (true) {
    if (mid >= oneOffEstimation) {
      highest = mid;
      if (
        ((previousHighest - highest) * ESTIMATION_VARIANCE) / previousHighest <
        1
      ) {
        break;
      }
...
function estimationVariance(binarySearchEstimation, oneOffEstimation) {
  return (
    ((binarySearchEstimation - oneOffEstimation) * ESTIMATION_VARIANCE) /
    binarySearchEstimation
  );
}
...
describeWithFrontier("Frontier RPC (Gas)", (context) => {
  ...
  it("eth_estimateGas for contract creation", async function () {
    ...
    expect(
      estimationVariance(binarySearchEstimation, oneOffEstimation),
    ).to.be.lessThan(1);
    expect(
  ...
  it("eth_estimateGas for contract call", async function () {
    ...
    expect(await contract.methods.multiply(3).estimateGas()).to.equal(
      binarySearchEstimation,
    );
  });
  ...
  it("eth_estimateGas should handle AccessList alias", async function () {
    ...
    expect(result).to.equal(
      context.web3.utils.numberToHex(binarySearchEstimation),
    );
  });
  ...
  it("tx gas limit below ETH_BLOCK_GAS_LIMIT", async function () {
    ...
    const createReceipt = await customRequest(
      context.web3,
...
describeWithFrontier(
  ...
  (context) => {
    ...
    const REMNANT = Math.floor(
      BLOCK_GAS_LIMIT - CALL_COST * (CALLS_PER_BLOCK - 1),
    );
    ...
    step("gas limit bound works with ref time heavy txns", async function () {
      ...
      expect(latest.transactions.length).to.be.eq(
        CALLS_PER_BLOCK + TRANSFERS_PER_BLOCK,
      );
...
describeWithFrontier(
  "Frontier RPC (Gas limit Weightv2 pov size)",
  (context) => {
    const STORAGE_LOOP_CONTRACT_BYTECODE = StorageLoop.bytecode;
    const STORAGE_LOOP_CONTRACT_ABI = StorageLoop.abi as AbiItem[];
    ...
    const BLOCK_GAS_LIMIT =
      ETH_BLOCK_GAS_LIMIT - (FIRST_CALL + CONTRACT_TRANSFER_EFFECTIVE_GAS);
    // Number of calls per block
    const CALLS_PER_BLOCK = Math.floor(BLOCK_GAS_LIMIT / CALL_COST) + 1; // +1 to count first call
    // Available space left after all calls
    const REMNANT = Math.floor(
...
describeWithFrontier(
  "Frontier RPC (Invalid opcode estimate gas)",
  (context) => {
    ...
    before(async () => {
      ...
      await createAndFinalizeBlock(context.web3);
      contractAddess = (await context.web3.eth.getTransactionReceipt(txHash))
        .contractAddress;
    });
    ...
    it("should estimate gas with invalid opcode", async function () {
      ...
      expect(estimate).to.equal(85704);
    });
  },
);
