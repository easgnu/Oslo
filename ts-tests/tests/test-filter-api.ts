...
describeWithFrontier("Frontier RPC (EthFilterApi)", (context) => {
  ...
  step("should create a Block filter and return the ID", async function () {
    let createFilter = await customRequest(
      context.web3,
      "eth_newBlockFilter",
      [],
    );
  ...
  step(
    "should return responses for pending transaction polling.",
    async function () {
      let poll = await customRequest(context.web3, "eth_getFilterChanges", [
        "0x4",
      ]);
  ...
  step("should return responses for Log filter polling.", async function () {
    ...
    expect(poll.result.length).to.be.eq(1);
    expect(poll.result[0].address.toLowerCase()).to.be.eq(
  ...
  step("should return response for raw Log filter request.", async function () {
    ...
    let poll = await customRequest(context.web3, "eth_getFilterLogs", [
      createFilter.result,
    ]);
    ...
    poll = await customRequest(context.web3, "eth_getFilterLogs", [
      createFilter.result,
    ]);
  ...
  step("should uninstall created filters.", async function () {
    ...
    let uninstall = await customRequest(context.web3, "eth_uninstallFilter", [
      filterId,
    ]);
    expect(uninstall.result).to.be.eq(true);

    // Should return error if does not exist.
    let r = await customRequest(context.web3, "eth_uninstallFilter", [
      filterId,
    ]);
  ...
  step("should drain the filter pool.", async function () {
    ...
    let r = await customRequest(context.web3, "eth_getFilterChanges", [
      filterId,
    ]);
    expect(r.error).to.include({
      message: "Filter id 7 does not exist.",
