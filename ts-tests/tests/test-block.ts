...
describeWithFrontier("Frontier RPC (Pending Block)", (context) => {
  ...
  it("should return pending block", async function () {
      ...
      const tx = await context.web3.eth.accounts.signTransaction(
        {
          ...
          to: TEST_ACCOUNT,
          value: "0x2000", // Must be higher than ExistentialDeposit
          gasPrice: "0x3B9ACA00",
          gas: "0x100000",
          nonce: nonce,
        },
    ...
    {
      const pending = (
        await customRequest(context.web3, "eth_getBlockByNumber", [
          "pending",
          false,
        ])
      ).result;
...
describeWithFrontier("Frontier RPC (BlockReceipts)", (context) => {
  ...
  it("should support block number, tag and hash", async function () {
    ...
    // block tags
    expect(
      (await customRequest(context.web3, "eth_getBlockReceipts", ["earliest"]))
        .result.length,
    ).to.be.eq(0);
    // expect((await customRequest(context.web3, "eth_getBlockReceipts", ["pending"])).result).to.be.null;
    expect(
      (await customRequest(context.web3, "eth_getBlockReceipts", ["finalized"]))
        .result.length,
    ).to.be.eq(N);
    expect(
      (await customRequest(context.web3, "eth_getBlockReceipts", ["latest"]))
        .result.length,
    ).to.be.eq(N);
  });
});
