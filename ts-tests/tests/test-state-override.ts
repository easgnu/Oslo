...
describeWithFrontier("Frontier RPC (StateOverride)", (context) => {
  ...
  before("create the contract", async function () {
    ...
    contract = new context.web3.eth.Contract(
      StateOverrideTest.abi as AbiItem[],
    );
    const data = contract
      .deploy({
  ...
  it("should have balance above 1000 tether without state override", async function () {
    ...
    const balance = Web3.utils.toBN(
      Web3.utils
        .fromWei(Web3.utils.hexToNumberString(result), "tether")
        .split(".")[0],
    );
    expect(balance.gten(1000), "balance was not above 1000 tether").to.be.true;
  });

  it("should have a sender balance of 5000 - EXISTENTIAL_DEPOSIT with state override", async function () {
    const { result } = await customRequest(context.web3, "eth_call", [
      {
        from: GENESIS_ACCOUNT,
        to: contractAddress,
    ...
    let senderBalance = 5000 - EXISTENTIAL_DEPOSIT;
    let numberString = senderBalance.toString();
    expect(Web3.utils.hexToNumberString(result)).to.equal(numberString);
  });
    ...
    const { result } = await customRequest(context.web3, "eth_call", [
      {
        ...
        data: contract.methods
          .allowance(GENESIS_ACCOUNT, otherAddress)
          .encodeABI(),
      },
    ]);
