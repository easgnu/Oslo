...
export const FRONTIER_BACKEND_TYPE = "key-value";
...
export async function createAndFinalizeBlock(
  ...
  const response = await customRequest(web3, "engine_createBlock", [
    true,
    finalize,
    null,
  ]);
...
export async function startFrontierNode(
  ...
  const binaryLogs = [];
  let spawnTimeout = SPAWNING_TIME - 2000;
...
export function describeWithFrontierFaTp(
  title: string,
  cb: (context: { web3: Web3 }) => void,
) {
...
export function describeWithFrontierSsTp(
  title: string,
  cb: (context: { web3: Web3 }) => void,
) {
...
export function describeWithFrontierAllPools(
  title: string,
  cb: (context: { web3: Web3 }) => void,
) {
...
export function describeWithFrontierWs(
  title: string,
  cb: (context: { web3: Web3 }) => void,
) {
