...
describeWithFrontier("Test self-destruct contract", (context) => {
  ...
  const TEST_CONTRACT_DEPLOYED_BYTECODE =
    SelfDestructAfterCreate2.deployedBytecode;
  ...
  it("SELFDESTRUCT must reset contract account", async function () {
    ...
    expect(
      await customRequest(context.web3, "eth_getCode", [
        FIRST_CONTRACT_ADDRESS,
      ]),
    ).to.deep.equal({
      id: 1,
    ...
    expect(
      await customRequest(context.web3, "eth_getCode", [deployedAddress]),
    ).to.deep.equal({
      id: 1,
      jsonrpc: "2.0",
      result: "0x",
