...
describeWithFrontier("Frontier RPC (Contract Methods)", (context) => {
	...
	it.skip("should fail for missing parameters", async function () {
		...
		await contract.methods.multiply().call().catch(
			(err) => expect(err.message).to.equal(`Returned error: VM Exception while processing transaction: revert.`)
		);
	});
	...
	// 		}
	// 	);
	// 	await contract.methods.multiply(3, 4).call().catch((err) =>
	...
	// 	);
	// 	await contract.methods.multiply("0x0123456789012345678901234567890123456789").call()
	// 		.catch((err) =>
	// 			expect(err.message).to.equal(`Returned error: VM Exception while processing transaction: revert.`)
	// 		);
	// });
