...
import { GENESIS_ACCOUNT, GENESIS_ACCOUNT_PRIVATE_KEY, CHAIN_ID, FIRST_CONTRACT_ADDRESS, EXISTENTIAL_DEPOSIT } from "./config";
...
describeWithFrontier("Frontier RPC (EIP-7702 Set Code Authorization)", (context: any) => {
	...
	step("happy path: complete EIP-7702 delegation workflow", async function () {
		...
		expect(BigInt(balance)).to.equal(BigInt(ethers.parseEther("1.0")) - BigInt(EXISTENTIAL_DEPOSIT));
