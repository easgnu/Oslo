...
function isDelegationIndicator(code: string): {
  ...
  if (
    code &&
    code.length === 48 &&
    code.startsWith(EIP7702_DELEGATION_PREFIX)
  ) {
...
describeWithFrontier(
  "Frontier RPC (EIP-7702 Set Code Authorization)",
  (context: any) => {
    ...
    step("should deploy delegate test contract", async () => {
      ...
      if (!contractAddress) {
        throw new Error(
          "Contract deployment failed: contractAddress is null or undefined",
        );
      }
    ...
    step(
      "should handle EIP-7702 transaction type 4 structure",
      async function () {
        ...
        if (!contractAddress) {
          throw new Error(
            "Contract address is required but not set from previous step",
          );
        }
        ...
        // Get current nonce
        const currentNonce =
          await context.ethersjs.getTransactionCount(GENESIS_ACCOUNT);
        ...
        const receipt = await context.ethersjs.getTransactionReceipt(
          signedTx.hash,
        );
    ...
    step(
      "should handle authorization with different chain IDs",
      async function () {
        this.timeout(15000);
        ...
        const receipt1 = await context.ethersjs.getTransactionReceipt(
          signedTx1.hash,
        );
        ...
        const receipt2 = await context.ethersjs.getTransactionReceipt(
          signedTx2.hash,
        );
    ...
    step(
      "should verify gas cost calculation includes authorization costs",
      async function () {
        this.timeout(15000);
        ...
        await createAndFinalizeBlock(context.web3);
        const regularReceipt = await context.ethersjs.getTransactionReceipt(
          regularSignedTx.hash,
        );
        ...
        await createAndFinalizeBlock(context.web3);
        const eip7702Receipt = await context.ethersjs.getTransactionReceipt(
          eip7702SignedTx.hash,
        );

        // EIP-7702 transaction should cost more gas due to authorization processing
        expect(Number(eip7702Receipt.gasUsed)).to.be.greaterThan(
          Number(regularReceipt.gasUsed),
        );
      },
    );
    ...
    step("should handle zero-address delegation", async function () {
      ...
      const zeroAuthorizerCode = await context.ethersjs.getCode(
        authorizer.address,
      );
    ...
    step(
      "happy path: complete EIP-7702 delegation workflow",
      async function () {
        this.timeout(20000);
        ...
        const fundingReceipt = await context.ethersjs.getTransactionReceipt(
          fundingTx.hash,
        );
        ...
        expect(BigInt(balance)).to.equal(
          BigInt(ethers.parseEther("1.0")) - BigInt(EXISTENTIAL_DEPOSIT),
        );

        // Step 3: Create authorization to delegate to the test contract
        const delegatorCurrentNonce =
          await context.ethersjs.getTransactionCount(delegatorAddress);
        ...
        const delegationReceipt = await context.ethersjs.getTransactionReceipt(
          signedDelegationTx.hash,
        );
        ...
        expect(delegationInfo.isDelegation).to.be.true;
        expect(delegationInfo.address.toLowerCase()).to.equal(
        ...
        const callReceipt = await context.ethersjs.getTransactionReceipt(
          callTx.hash,
        );
        ...
        const finalBalance =
          await context.web3.eth.getBalance(delegatorAddress);
    ...
    step("should estimate gas for EIP-7702 transactions", async function () {
      ...
      const regularTestGasEstimate =
        await context.ethersjs.estimateGas(regularTestTx);
      console.log(
        "Regular tx gas estimate:",
        regularTestGasEstimate.toString(),
      );
      ...
      console.log(
        "Sending actual EIP-7702 transaction first to verify it works...",
      );
      const actualTx = {
        from: GENESIS_ACCOUNT,
        to: "0x1000000000000000000000000000000000000001",
      ...
      await createAndFinalizeBlock(context.web3);
      const txReceipt = await context.ethersjs.getTransactionReceipt(
        sentTx.hash,
      );
      console.log(
        "EIP-7702 tx succeeded with gas used:",
      ...
      try {
        const runtimeVersion = await customRequest(
          context.web3,
      ...
      try {
        ...
        const emptyAuthEstimate = await customRequest(
          context.web3,
          "eth_estimateGas",
          [emptyAuthTx],
        );
        console.log(
          "Empty authorization list gas estimate:",
          emptyAuthEstimate,
        );
      } catch (error) {
      ...
      const web3GasEstimate = await customRequest(
        context.web3,
        "eth_estimateGas",
        [web3TxParams],
      );
    ...
    step(
      "should handle gas estimation edge cases for EIP-7702",
      async function () {
        // Ensure we have a signer
        if (!signer) {
          signer = new ethers.Wallet(
        ...
        try {
          wrongChainGasEstimate =
            await context.ethersjs.estimateGas(wrongChainTx);
          console.log(
            "Gas estimate with wrong chain ID auth:",
            wrongChainGasEstimate.toString(),
          );
        } catch (error) {
          console.log(
            "Wrong chain gas estimation failed, using fallback:",
            error.message,
          );
        ...
        try {
          selfDelegationGasEstimate =
            await context.ethersjs.estimateGas(selfDelegationTx);
          console.log(
            "Gas estimate for self-delegation:",
            selfDelegationGasEstimate.toString(),
          );
        } catch (error) {
          console.log(
            "Self-delegation gas estimation failed, using fallback:",
            error.message,
          );
        ...
        try {
          zeroAddressGasEstimate =
            await context.ethersjs.estimateGas(zeroAddressTx);
          console.log(
            "Gas estimate for zero address delegation:",
            zeroAddressGasEstimate.toString(),
          );
        } catch (error) {
          console.log(
            "Zero address gas estimation failed, using fallback:",
            error.message,
          );
        ...
        try {
          highNonceGasEstimate =
            await context.ethersjs.estimateGas(highNonceTx);
          console.log(
            "Gas estimate with high nonce auth:",
            highNonceGasEstimate.toString(),
          );
        } catch (error) {
          console.log(
            "High nonce gas estimation failed, using fallback:",
            error.message,
          );
